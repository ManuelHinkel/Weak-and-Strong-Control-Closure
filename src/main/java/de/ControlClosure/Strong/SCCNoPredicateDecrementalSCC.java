...
public class SCCNoPredicateDecrementalSCC implements StrongControlClosureNoPredicate{
    private final DecrementalSCC dscc;

    public SCCNoPredicateDecrementalSCC(DecrementalSCC dscc) {
        this.dscc = dscc;
    }

    @Override
    public Set<Vertex> scc(Graph<Vertex> G, Set<Vertex> Vp, Set<Vertex> F) {
        ...
        while ((scc = dscc.SCCs().prev(lastMoved)) != null){
            ...
            if (ThetaHatSCC.size() >= 2
                ...
                || (ThetaHatSCC.size() == 1 && F.contains(SetUtils.getFirst(Boundary.get(scc)))) // incomplete predicate
            ...
            { // O(1)
                ...
                if (scc.size() > 1
                        ...
                        || (!X.contains(scc.first()) && F.contains(scc.first()))) {
                    for(Vertex v: scc.vertices()) {
                        for(Vertex u: H.incoming(v)) {
                            SCC<Vertex> sccInc = dscc.scc(u); // O(1)
                            if (!scc.equals(sccInc)) { // O(1)
